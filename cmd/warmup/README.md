...
| 模組 | 耗時 | 資料量 | 說明 |
|------|------|--------|------|
| **ID** | 3-5 分鐘 | 1-2 萬筆 | 系所代碼、近 5 年學生（108-112 學年） |
| **Contact** | 2-3 分鐘 | 500-1000 筆 | 行政與學術單位聯絡資訊 |
| **Course** | 5-10 分鐘 | 5000-1 萬筆 | 近 3 年課程（U/M/N/P 學制） |
| **總計** | **10-18 分鐘** | **~2.4 萬筆** | |
...
```bash
# 首次部署
go run ./cmd/warmup -reset -workers=10
...
| 問題 | 原因 | 解決方法 |
|------|------|----------|
| 爬蟲失敗 | NTPU 網站無法連線或限流 | 降低 workers (`-workers=3`)、稍後重試 |
| 預熱過慢 | Worker 太少或網路延遲 | 增加 workers (`-workers=15`)、離峰執行 |
| Database locked | 服務正在使用資料庫 | 停止服務後再執行 warmup |
| 記憶體不足 | 並發數過高 | 降低 workers (`-workers=5`) |
...
測試環境：Intel i7, 50 Mbps, -workers=10

| 設定 | 總耗時 | 說明 |
|------|--------|------|
| workers=5 | 18-20 分鐘 | 保守，預設值 |
| workers=10 | 12-15 分鐘 | 平衡 |
| workers=20 | 8-10 分鐘 | 激進，可能觸發限流 |
