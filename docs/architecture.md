...
│  • Per-domain rate limiting (5rps) │ │  • Pure Go(Memory Index) │
│  • Timeout: 60s per request        │ │  • Chinese Segmenter(gse)│
...
- **Info**：使用者操作與結果（訊息/按鈕進入、意圖分派、查詢開始與結果摘要）。
- **Debug**：內部細節與診斷資訊（演算法細節、字元比對、擴展查詢、重試細節）。
...
- `server_name`：節點名稱（`NTPU_SERVER_NAME` 或自動偵測）。
- `instance_id`：實例識別（`NTPU_INSTANCE_ID` 或自動偵測）。
...
- `server_name` 來源（未設定時）：`NODE_NAME` / `K8S_NODE_NAME` / `KUBE_NODE_NAME` / `MY_NODE_NAME` → hostname →（最後）`instance_id`。
- `instance_id` 來源（未設定時）：`POD_UID` / `MY_POD_UID` / `POD_NAME` / `MY_POD_NAME` / `HOSTNAME` → hostname →（最後）`server_name`。
...
func (r *Registry) DispatchMessage(ctx context.Context, text string) ([]msgs, string) {
    // Strategy pattern: 依關鍵字選擇處理器
    for _, handler := range r.handlers {
        if handler.CanHandle(text) {
            return handler.HandleMessage(ctx, text), handler.Name()
        }
    }
