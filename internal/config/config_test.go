	...
	tests := []struct {
		...
		// R2 Tests
		{
			...
			cfg: &Config{
				...
				R2LockKey:              "locks/leader.json",
				R2LockTTL:              time.Hour,
				R2SnapshotPollInterval: 5 * time.Minute,
				R2DeltaPrefix:          "deltas",
				R2ScheduleKey:          "schedules/maintenance.json",
			},
			wantErr:     true,
		...
		{
			name: "R2 enabled but empty schedule key",
			cfg: &Config{
				LineChannelToken:           "token",
				LineChannelSecret:          "secret",
				Port:                       "10000",
			...
			wantErr:     true,
			errContains: "NTPU_R2_SCHEDULE_KEY",
		},
		{
